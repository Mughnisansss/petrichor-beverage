...
type StorageMode = 'local' | 'server' | 'database';
...
const apiService = {
  ...
  addSale: async (sale: Omit<Sale, 'id' | 'date'>): Promise<Sale> => {
    const saleWithOptionalUser = { ...sale, createdById: undefined }; // Optional for backward compatibility
    return (await fetch('/api/sales', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(saleWithOptionalUser) })).json();
  },
  ...
  batchAddSales: async (sales: Omit<Sale, 'id' | 'date'>[]): Promise<Sale[]> => {
    const salesWithOptionalUser = sales.map(sale => ({ ...sale, createdById: undefined })); // Optional for backward compatibility
    return (await fetch('/api/sales/bulk', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(salesWithOptionalUser) })).json();
  },
  addOperationalCost: async (cost: Omit<OperationalCost, 'id' | 'date'>): Promise<OperationalCost> => {
    const costWithOptionalUser = { ...cost, createdById: undefined }; // Optional for backward compatibility
    return (await fetch('/api/operasional', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(costWithOptionalUser) })).json();
  },
...
const getLocalData = (): DbData => {
  if (typeof window === 'undefined') {
    return { appName: 'Petrichor', logoImageUri: null, marqueeText: 'Welcome!', themeId: 'default', drinks: [], foods: [], sales: [], operationalCosts: [], rawMaterials: [] };
  }
  ...
  try {
    ...
    parsedData.themeId = parsedData.themeId || 'default';
...
const databaseServiceIntegration = {
  getData: async (): Promise<DbData> => {
    try {
      return await databaseService.getAllData();
    } catch (error) {
  ...
  importData: async (data: DbData): Promise<{ ok: boolean, message: string }> => {
    try {
      ...
      return { ok: false, message: 'Failed to import data to database' };
    }
  },
  addDrink: async (drink: Omit<Drink, 'id' | 'costPrice'>): Promise<Drink> => {
    return await databaseService.addDrink(drink);
  },
  updateDrink: async (id: string, drink: Partial<Omit<Drink, 'id'>>): Promise<Drink> => {
    return await databaseService.updateDrink(id, drink);
  },
  deleteDrink: async (id: string) => {
    return await databaseService.deleteDrink(id);
  },
  addFood: async (food: Omit<Food, 'id' | 'costPrice'>): Promise<Food> => {
    return await databaseService.addFood(food);
  },
  updateFood: async (id: string, food: Partial<Omit<Food, 'id'>>): Promise<Food> => {
    return await databaseService.updateFood(id, food);
  },
  deleteFood: async (id: string) => {
    return await databaseService.deleteFood(id);
  },
  addSale: async (sale: Omit<Sale, 'id' | 'date'>): Promise<Sale> => {
    return await databaseService.addSale(sale);
  },
  deleteSale: async (id: string) => {
    return await databaseService.deleteSale(id);
  },
  batchAddSales: async (sales: Omit<Sale, 'id' | 'date'>[]): Promise<Sale[]> => {
    return await databaseService.batchAddSales(sales);
  },
  addOperationalCost: async (cost: Omit<OperationalCost, 'id' | 'date'>): Promise<OperationalCost> => {
    return await databaseService.addOperationalCost(cost);
  },
  updateOperationalCost: async (id: string, cost: Omit<OperationalCost, 'id' | 'date'>): Promise<OperationalCost> => {
    return await databaseService.updateOperationalCost(id, cost);
  },
  deleteOperationalCost: async (id: string) => {
    return await databaseService.deleteOperationalCost(id);
  },
  addRawMaterial: async (material: Omit<RawMaterial, 'id'>): Promise<RawMaterial> => {
    return await databaseService.addRawMaterial(material);
  },
  updateRawMaterial: async (id: string, material: Omit<RawMaterial, 'id'>): Promise<RawMaterial> => {
    return await databaseService.updateRawMaterial(id, material);
  },
  deleteRawMaterial: async (id: string) => {
    return await databaseService.deleteRawMaterial(id);
  },
  importRawMaterialsFromCsv: async (materials: Omit<RawMaterial, 'id'>[]): Promise<RawMaterial[]> => {
    return await databaseService.importRawMaterialsFromCsv(materials);
  },
  importOperationalCostsFromCsv: async (costs: Omit<OperationalCost, 'id'>[]): Promise<OperationalCost[]> => {
    return await databaseService.importOperationalCostsFromCsv(costs);
  },
};
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const [dbData, setDbData] = useState<DbData>({ appName: 'Petrichor', logoImageUri: null, marqueeText: '', themeId: 'default', drinks: [], foods: [], sales: [], operationalCosts: [], rawMaterials: [] });
  const [isLoading, setIsLoading] = useState(true);

  const currentService = useMemo(() => {
    if (storageMode === 'local') return localStorageService;
    if (storageMode === 'database') return databaseServiceIntegration;
    return apiService;
  }, [storageMode]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const { appName, logoImageUri, marqueeText, themeId, ...restOfDbData } = data;
      ...
      if (themeId) restOfDbData.themeId = themeId;
      ...
      setDbData({ appName: 'Petrichor', logoImageUri: null, marqueeText: '', themeId: 'default', drinks: [], foods: [], sales: [], operationalCosts: [], rawMaterials: [] });
    } finally {
        setIsLoading(false);
    }
  ...
  const contextValue = useMemo(() => {
    ...
    return {
      ...
      marqueeText,
      setMarqueeText,
      themeId: dbData.themeId || 'default',
      setThemeId: (themeId: string) => {
        setDbData(prev => ({ ...prev, themeId }));
      },
      fetchData,
      importData,
      addToCart,
