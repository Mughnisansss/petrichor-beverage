...
const DrinkForm = React.forwardRef(({ onFinished }: { onFinished: () => void }, ref) => {
    ...
    async function onSubmit(values: DrinkFormValues) {
        try {
            const drinkData = { ...values, costPrice: calculatedCostPrice };
            if (editingDrink) {
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <FormField control={form.control} name="name" render={({ field }) => (
                        <FormItem><FormLabel>Nama Minuman</FormLabel><FormControl><Input {...field} placeholder="cth: Es Kopi Susu" /></FormControl><FormMessage /></FormItem>
                    )}/>
                    <FormField control={form.control} name="sellingPrice" render={({ field }) => (
                        <FormItem><FormLabel>Harga Jual</FormLabel><FormControl><Input type="number" {...field} placeholder="cth: 18000" /></FormControl><FormMessage /></FormItem>
                    )}/>
                </div>
...
export default function MinumanPage() {
    const { drinks, deleteDrink } = useAppContext();
    const { toast } = useToast();
    const [isFormVisible, setFormVisible] = useState(false);
    const formRef = React.useRef<{ handleEdit: (drink: Drink) => void }>(null);


    const handleDelete = async (id: string) => {
        if (!window.confirm("Apakah Anda yakin ingin menghapus minuman ini? Riwayat penjualan tidak akan terhapus.")) return;
        try {
        ...
        <div className="flex flex-col gap-8">
            <div className="flex justify-between items-center">
                ...
                <Button onClick={() => setFormVisible(!isFormVisible)}>
                    <PlusCircle className="mr-2 h-4 w-4" /> {isFormVisible ? "Tutup Form" : "Tambah/Edit Minuman"}
                </Button>
            </div>
            ...
            {isFormVisible && (
                <Card>
                    <CardHeader>
                         <CardTitle>Form Minuman</CardTitle>
            ...
            <Card>
                <CardHeader><CardTitle>Daftar Minuman</CardTitle></CardHeader>
                <CardContent>
                    <Table>
                        <TableHeader>
                            <TableRow>
                            ...
                            drinks.map(drink => (
                                <TableRow key={drink.id}>
                                    ...
                                    <TableCell className="flex gap-2 justify-end">
                                        <Button variant="outline" size="icon" onClick={() => handleEditClick(drink)}>
